...
export function setup_routes(app: any, io: any) {
    ...
    app.get('/api/apply_suggestions', async (req, res) => {
        ...
        const latestSuggestion = await queryLogger.getLatestUnappliedSuggestion(parseInt(queryId));

        if (!latestSuggestion || !latestSuggestion.suggested_indexes) {
            res.json(stats);
            return;
        }

        const suggested_indexes = latestSuggestion.suggested_indexes;
        ...
        try {
            await queryLogger.markSuggestionApplied(latestSuggestion.suggestion_id, {
                prev_exec_time,
                new_exec_time: execTime
            });
        } catch (err) {
    ...
    app.get('/api/revert_suggestions', async (req, res) => {
        ...
        const latestApplied = await queryLogger.getLatestAppliedSuggestion(parseInt(queryId));

        const applied_indexes = latestApplied?.suggested_indexes || stats.applied_indexes;
        ...
        if (latestApplied) {
            try {
                await queryLogger.markSuggestionReverted(latestApplied.suggestion_id);
            } catch (err) {
